...
## Version of Node used `18.16.0`

## My Thoughts

1. Create-React-App and Node in version `14.17.3` gave me lot of problems, with installs and running up, so just used Vite+React+TS boilerplate, made changes to `Dockerfile` to use Image of Node version `18.16.0`, also needed to add `platform: linux/x86_64` to `docker-compose.yml` because otherwise couldn't run `docker-compose`
   &nbsp;
2. Tried to not touch UI at all, but couldn't help myself and added Error page, also added button to edit article, improved components a little bit, made them little bit less bloated, in some places I have made some changes, e.g extracted `LoginRegister` component into `Login` and `Register` components and used them appropriately for `Login` and `Register` routes, I have also moved elements for example that were representing `nav` or `footer` in every single page to it's own `Navbar` and `Footer` components, for better re-usability, of course I could've gone much further but I guess that's no really the point of this assignment.
   &nbsp;
3. Mismatches between requirements and what api offers, for example there is requirement that says `Button to follow author (with followers count)`, but api offers only `followed` which is `boolean`
   &nbsp;
4. Sorry for no unit tests, I don't have much experience in testing.
